  ...
  return (
    <div ref={ref} className="mx-auto max-w-5xl space-y-8 px-4 py-10 md:px-8">
      <div className="relative overflow-hidden">
        ...
        <div
          className="orb -top-10 right-24 h-44 w-44 bg-sky-500/15"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
        <Card interactive className="rounded-2xl p-3">
          <div className="mb-1 grid grid-cols-7">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
              <div
                key={d}
                className="text-text-muted py-1 text-center text-[10px] tracking-wider uppercase"
              >
                {d}
              </div>
            ))}
            ...
            {grid.map((day, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex aspect-square flex-col items-center justify-start rounded-lg p-1 text-xs transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]",
                    isSelected
                      ? "bg-white/[0.08] ring-1 ring-emerald-400/30"
                      : "hover:bg-white/[0.04]"
                  )}
                >
                  <span
                    className={cn(
                      ...
                      isToday ? "bg-emerald-400 font-semibold text-black" : "text-text-secondary"
                    )}
                  >
                    {day.getDate()}
                  </span>
        ...
        <div className="space-y-2">
          ...
          {dayEvents.length === 0 ? (
            ...
            dayEvents.map((e) => (
              <Card key={e.id} interactive className="group rounded-xl py-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-text-primary text-sm font-medium">{e.title}</p>
                    <p className="text-text-muted mt-0.5 flex items-center gap-1 text-[11px]">
                      <Clock size={10} />
                  ...
                  <button
                    onClick={() => remove(e.id)}
                    className="text-text-muted opacity-0 group-hover:opacity-100 hover:text-rose-400"
                    aria-label="Delete"
                  >
                    <Trash2 size={12} />
                  </button>
    ...
    <Dialog open={open} onClose={onClose} title="New event">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-text-muted mb-1 block text-[10px] uppercase">Start</label>
            <input
              type="datetime-local"
              value={start}
              onChange={(e) => setStart(e.target.value)}
              className="glass-input h-9 w-full rounded-md px-2 text-xs"
            />
          </div>
          <div>
            <label className="text-text-muted mb-1 block text-[10px] uppercase">End</label>
            <input
              type="datetime-local"
              value={end}
