...
function GitHubIntegrationInner() {
    ...
    if (err) {
      const messages: Record<string, string> = {
        ...
        token_exchange_failed:
          "Failed to exchange code for token. Check GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET.",
      };
  ...
  const handleOAuth = () => {
    window.location.href =
      "/api/oauth/github/authorize?redirect_to=" + encodeURIComponent(window.location.pathname);
  };
    ...
    <div ref={ref} className="space-y-8">
      <div className="relative isolate py-10">
        ...
        <div className="relative">
          ...
          <h1 className="display text-gradient mt-3 text-4xl md:text-5xl">GitHub</h1>
          <p className="text-text-secondary mt-3 max-w-2xl text-sm">
            Connect your GitHub account to let the agent manage repositories, issues, pull requests,
            and read code.
          </p>
        </div>
      </div>
      ...
      {!loading && active && (
        <>
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-text-primary text-sm font-medium">{active.githubUser}</p>
                  ...
                  <p className="text-text-muted mt-0.5 text-[11px]">
                    ...
                    {active.lastSyncedAt
                      ? ` · Last synced ${timeAgo(active.lastSyncedAt)}`
                      : " · Not synced yet"}
                  </p>
                </div>
              </div>
          ...
          {repos.length > 0 && (
            <>
              <p className="text-text-muted mt-6 text-[10px] tracking-wider uppercase">
              ...
              <div className="space-y-1">
                {repos.slice(0, 30).map((repo) => (
                  <a
                    ...
                    className="block"
                  >
                    <Card interactive className="flex items-center gap-3 rounded-xl px-3 py-2.5">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-text-primary font-mono text-xs">
                            {repo.fullName}
                          </span>
                          <span className="text-text-muted rounded bg-white/5 px-1.5 py-0.5 text-[9px] font-semibold uppercase">
                        ...
                        <p className="text-text-muted mt-0.5 text-[11px]">
                          ...
                          {repo.language && <>{repo.language} · </>}★ {repo.stars ?? 0}
                        </p>
                      </div>
          ...
          {repos.length === 0 && (
            <Card className="rounded-2xl py-6 text-center">
              <p className="text-text-secondary text-xs">
                No synced repos. Click <RefreshCw size={12} className="inline text-emerald-400" />{" "}
                to sync your repositories.
              </p>
            </Card>
          )}
          ...
          <Card className="space-y-1 rounded-2xl">
            ...
            <ToolToggle
              ...
              setChecked={(v) => {
                setApproveIssue(v);
                saveToggle("approve_createIssue", v);
              }}
            />
            <ToolToggle
              label="createPR"
            ...
            <ToolToggle
              ...
              setChecked={(v) => {
                setApproveListRepos(v);
                saveToggle("approve_listRepos", v);
              }}
            />
          </Card>
        </>
      ...
      {!loading && !active && (
        <>
          ...
          <Card className="rounded-2xl py-10 text-center">
            <Github size={32} className="text-text-muted mx-auto mb-3" />
            <p className="text-text-secondary mb-4 text-sm">
              No GitHub account connected. OAuth is required to let the agent interact with your
              repos.
            </p>
