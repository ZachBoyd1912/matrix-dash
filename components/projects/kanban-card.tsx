...
const PROJECT_COLORS: Record<string, { border: string; badge: string; bg: string }> = {
  ...
  "make-blueprints-ready": {
    ...
    bg: "hover:bg-teal-400/[0.04]",
  },
  "the-greater-flaw": {
    border: "border-l-slate-400",
    badge: "bg-slate-400/15 text-slate-300",
    bg: "hover:bg-slate-400/[0.04]",
  },
};
...
export const KanbanCard = memo(function KanbanCard({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "group relative rounded-lg border border-l-[3px] border-white/[0.06] transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]",
        "bg-white/[0.03] backdrop-blur-sm",
        "hover:translate-y-[-1px] hover:border-white/[0.12] hover:bg-white/[0.05] hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.4)]",
        colors?.border ?? "border-l-white/20",
        colors?.bg ?? "",
        isDragging &&
          "z-50 scale-[1.02] rotate-[1deg] opacity-40 shadow-[0_12px_40px_-8px_rgba(0,0,0,0.6)]",
      ...
      <div className="px-2.5 py-2">
        ...
        <div className="mb-1.5 flex items-center justify-between gap-1">
          <div className="flex min-w-0 items-center gap-1">
            ...
            <span
              className={cn(
                "flex shrink-0 items-center gap-0.5 rounded-full px-1 py-[1px] text-[8px] font-semibold tracking-wider uppercase",
                kind.className
              )}
              title={kind.label}
            >
            ...
            {project && (
              <span
                className={cn(
                  "truncate rounded-full px-1.5 py-[1px] text-[8px] font-semibold tracking-wider uppercase",
                  colors?.badge ?? "text-text-muted bg-white/5"
                )}
              >
                {project.name.length > 14 ? project.name.slice(0, 12) + "…" : project.name}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
            <button
              className="text-text-muted/40 -ml-0.5 cursor-grab p-0 hover:text-white active:cursor-grabbing"
              {...attributes}
              {...listeners}
            ...
            <button
              ...
              className={cn(
                ...
                prevColumn
                  ? "text-text-muted/50 hover:bg-white/10 hover:text-white"
        ...
        {editing ? (
          <div className="flex items-center gap-1">
            <input
              ...
              onBlur={() => saveEdit()}
              className="glass-input text-text-primary h-5 w-full rounded px-1.5 text-[12px] font-medium outline-none"
              autoFocus
              onClick={(e) => e.stopPropagation()}
            />
            <button
              ...
              disabled={saving}
              className="shrink-0 rounded p-0.5 text-emerald-400 hover:text-emerald-300"
              aria-label="Save"
            >
              <Check size={10} />
            </button>
            <button
              ...
              className="text-text-muted shrink-0 rounded p-0.5 hover:text-rose-400"
              aria-label="Cancel"
            >
              <X size={10} />
            </button>
          ...
          <p
            onClick={startEdit}
            className="text-text-primary line-clamp-2 cursor-text text-[12px] leading-snug font-medium transition-colors hover:text-emerald-300/80"
            title="Click to edit"
          >
            {task.title}
          </p>
        ...
        {/* ── Description ── */}
        {!editing && description && (
          <p className="text-text-muted/80 mt-1 line-clamp-2 text-[10px] leading-snug">
            {description}
          </p>
        )}
        ...
        {(dueLabel || task.priority !== "normal") && (
          <div className="mt-1.5 flex items-center gap-2 text-[9px]">
            {dueLabel && (
              <span
                className={cn(
                  ...
                  dueOverdue && "font-medium text-rose-400",
                  dueToday && !dueOverdue && "font-medium text-amber-400",
            ...
            {task.priority !== "normal" && (
              <span
                className={cn("ml-auto flex items-center gap-0.5", PRIORITY_COLORS[task.priority])}
              >
                <AlertCircle size={8} />
                {task.priority}
              </span>
